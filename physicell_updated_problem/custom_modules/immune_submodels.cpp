...
void CD8_Tcell_phenotype( Cell* pCell, Phenotype& phenotype, double dt )
{
    ...
    //rwh
	std::cout<< __FUNCTION__ << ": elapsed_time_in_phase= " << pCell->phenotype.cycle.data.elapsed_time_in_phase << ", current_phase_index= " <<  pCell->phenotype.cycle.data.current_phase_index << std::endl;
	...
	if(pCell->phenotype.cycle.data.elapsed_time_in_phase<6 &&  pCell->phenotype.cycle.data.current_phase_index==0)
	{
	    ...
	    std::cout<< __FUNCTION__ << ": decrementing generation --> " << pCell->custom_data["generation"] << std::endl;
	}
